...
namespace bmctelemetry
{
    ...
    struct OtelMetrics
    {
        ...
        void addCounterView(const std::string& name, const std::string& version, const std::string& schema){
        ...
        void addObservableCounterView(const std::string& name,  const std::string& version, const std::string& schema)
        {
        ...
        void addHistogramView(const std::string& name, const std::string& version, const std::string& schema)
        {
        ...
        auto createDoubleCounter(const std::string& name){
            nostd::shared_ptr<metrics_api::Meter> meter = p->GetMeter(name, "1.2.0");
        ...
        auto createDoubleHistogram(const std::string& name, const std::string& description, const std::string& unit){
            nostd::shared_ptr<metrics_api::Meter> meter = p->GetMeter(name, "1.2.0");
...
#define TRACE_FUNCION auto func_span=bmctelemetry::trace::Scope(bmctelemetry::get_tracer()->StartSpan(__FUNCTION__));
#define START_TRACE(X) auto X=bmctelemetry::trace::Scope(bmctelemetry::get_tracer()->StartSpan(#X));
